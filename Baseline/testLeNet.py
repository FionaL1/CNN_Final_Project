...
def main():
  ...
  xtrain, ytrain, xval, yval, xtest, ytest = cnn_lenet_np.load_mnist(fullset)
  ...
  params = cnn_lenet_np.init_convnet(layers)
  ...
  for step in range(max_iter):
    ...
    startTime = time.time()
    [cp, param_grad] = cnn_lenet_np.conv_net(params,
                                          layers,
                                          xtrain[:, idx],
                                          ytrain[idx], True)
    ...
    w_rate = cnn_lenet_np.get_lr(step, epsilon*w_lr, gamma, power)
    b_rate = cnn_lenet_np.get_lr(step, epsilon*b_lr, gamma, power)
    params, param_winc = cnn_lenet_np.sgd_momentum(w_rate,
                           b_rate,
                           mu,
                           weight_decay,
                           params,
    ...
    if (step+1) % test_interval == 0:
      ...
      cptest, _ = cnn_lenet_np.conv_net(params, layers, xtest, ytest, False)
      layers[1]['batch_size'] = 64
