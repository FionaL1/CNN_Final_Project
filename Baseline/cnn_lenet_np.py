...
def colImg2maxPoolImg(img, img_height, img_width, k, s, done_height, done_width): 
  """Pooling forward helper function

  Args:
    img: a img_height*img_width by 1 image with img_height and img_width
    k: sample matrix size
    s: sample stride
    done_height: the height of the max pool matrix
    done_width: the height of the max pool matrix

  Returns:
    output: a max pooled img whose dimension is done_height*done_width by 1
  """
  new_width = k*k
  new_height = ((img_height - k )/s + 1) * ((img_width - k )/s + 1)
  new_matrix = np.ones([new_height, new_width])
  for i in range(new_height):
      indexes = new_width / k
      for j in range(indexes):
          offset_1 = (i / done_width)*s
          offset_2 = (i % done_width)*s
          update_index = (j+offset_1)*img_width
          update_with_offset = update_index + offset_2

          max_pool_img_block = img[update_with_offset: update_with_offset+k]
          new_matrix[i][j*k:(j+1)*k] = np.transpose(max_pool_img_block)
  new_matrix = np.amax(new_matrix, axis=1)
  new_matrix = np.asarray(new_matrix).ravel()
  # new_matrix = np.reshape(new_matrix, (new_height,1))
  return new_matrix
...
def pooling_layer_forward(input, layer):
  ...
  input_img2col_conv = {}
  input_img2col_conv['height'] = h_in
  input_img2col_conv['width'] = w_in
  input_img2col_conv['channel'] = c

  for img_index in range(batch_size):
    input_img2col_conv['data'] = input['data'][:, img_index]
    max_pooled_blocks = im2col_conv(input_img2col_conv, layer, h_out, w_out)
    max_pooled_blocks = np.reshape(max_pooled_blocks, (k*k,c,h_out*w_out))
    max_pooled_blocks = np.amax(max_pooled_blocks, axis = 0)
    max_pooled_blocks = max_pooled_blocks.flatten('F')
    output['data'][:, img_index] = max_pooled_blocks
  # implementation ends
...
def relu_forward(input, layer):
  ...
  # implementation begins
  output['data'] = input['data'].clip(min = 0)
...
def inner_product_forward(input, layer, param):
  ...
  # implementation begins
  param_b = np.tile(param['b'], (input['data'].shape[1],1))
  input_trans = np.transpose(input['data'])
  WX = np.dot(input_trans, param['w']) 
  WX_B = WX + param_b
  output['data'] = np.transpose(WX_B)
  # implementation ends
